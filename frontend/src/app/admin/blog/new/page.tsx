...
export default function NewPostPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const res = await api.post("/posts/admin", form);
      if (res.data.success) {
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. 5 Tips for Choosing the Right Sofa"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Slug (URL) *
                <span className="text-xs text-gray-400 font-normal ml-2">— auto-generated, editable</span>
              </label>
              <input
                required
                value={form.slug}
                ...
                placeholder="e.g. 5-tips-choosing-right-sofa"
              />
            </div>
            <div>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-4">
            ...
            <p className="text-xs text-gray-400">
              Unchecked = Draft (not visible on the blog page)
            </p>
          </div>
        </div>
      </div>
